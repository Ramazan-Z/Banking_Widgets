...
from src.processing import filter_by_line_in_description, filter_state_operations, sorted_date_operacions
...
def state_selection(transactions: list[dict]) -> list[dict]:
    ...
    while True:
        ...
        if answer in ("EXECUTED", "CANCELED", "PENDING"):
            ...
            return filter_state_operations(transactions, answer)
        else:
        ...
        if answer in ("ДА", "Д", "YES", "Y"):
            while True:
                ...
                if answer in ("ПО ВОЗРАСТАНИЮ", "ВОЗРАСТАНИЮ", "ВОЗРАСТАНИЕ"):
                    ...
                    return sorted_date_operacions(transactions, False)
                elif answer in ("ПО УБЫВАНИЮ", "УБЫВАНИЮ", "УБЫВАНИЕ"):
                    print("Выбрана сортировка по убыванию.\n")
                    return sorted_date_operacions(transactions, True)
                else:
